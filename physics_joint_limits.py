...
def _E_body_zyx_limits(angles):
    """
    Angular velocity Jacobian in the child body LOCAL frame for ZYX Euler
    angles [alpha(Z), beta(Y), gamma(X)].  Duplicated here (from physics_engine)
...
def compute_joint_limit_torques(engine) -> np.ndarray:
    ...
    For quaternion spherical joints (dof=4):
      - ZYX Euler angles are extracted from the stored quaternion.
      - Penalty torques are computed in the ZYX Euler basis then projected
        back to the local-omega basis via the inverse of E_body_zyx.
      - qdot[s:s+3] = omega_local (body frame), so angular velocity
        per-axis is extracted as: euler_dot = E_body_inv @ omega_local.

    For legacy Euler spherical joints (dof=3): unchanged behaviour.
    ...
    for (midx, jname, jinfo, dof) in engine.joint_list:
        ...
        if dof == 4:
            # --- Quaternion spherical joint ---
            # Extract quaternion and convert to ZYX Euler angles.
            qjt = q[s:s+4]
            ...
            # omega_local = qdot[s:s+3]; map to Euler-dot via E_body_inv.
            # E_body @ euler_dot = omega_local  =>  euler_dot = pinv(E_body) @ omega_local
            omega_local = qdot[s:s+3]
            E  = _E_body_zyx_limits(euler_angles)
            # E is 3x3; use lstsq for numerical stability near singularity
            euler_dot, _, _, _ = np.linalg.lstsq(E, omega_local, rcond=None)

            # Compute penalty torques in Euler-dot space, then project to omega_local space.
            tau_euler = np.zeros(3)
            for (_, dof_idx, lo, hi) in rom['axes']:
                if dof_idx >= 3:
                    continue
                theta  = euler_angles[dof_idx]
                thetad = euler_dot[dof_idx]
                if theta > hi:
                    violation = theta - hi
                    tau_euler[dof_idx] -= k * violation + c * max(0.0, thetad)
                elif theta < lo:
                    violation = lo - theta
                    tau_euler[dof_idx] += k * violation - c * min(0.0, thetad)

            # Project: Q_omega = E^T @ tau_euler  (virtual-power duality)
            Q[s:s+3] += E.T @ tau_euler

        elif dof == 3:
            # --- Legacy Euler spherical joint ---
            for (_, dof_idx, lo, hi) in rom['axes']:
                if dof_idx >= dof:
                    continue
                theta  = q[s + dof_idx]
