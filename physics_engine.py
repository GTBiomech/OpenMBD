...
class PhysicsEngine:
    ...
    def __init__(self):
        self.models = []
        self.model_configs = []# physics_engine.py  
# Citation: Tierney. OpenMBD: An Open-Source Multibody Dynamics Simulator for Biomechanics Research and Education. F1000Research, 2026.
# Version: 1.1 
# Research Contact: Dr Gregory Tierney (g.tierney@ulster.ac.uk)
    ...
    def step(self):
        ...
        for (midx, jname, jinfo, dof) in self.joint_list:
            ...
            if jinfo.get('is_root_joint', False) and dof == 7:
                ...
                # Quaternion: qdot_quat = 0.5 * [0, omega_world] (x) q   (LEFT Hamilton
                # product -- omega_world must pre-multiply q, since qdot[s+3:s+6] for
                # the free root is WORLD-frame omega everywhere else in this file, e.g.
                # rnea()'s "omega[i] = qd[3:6]" and compute_a1_a2_analytic's
                # "A2[r,:,s+3:s+6] = np.eye(3)". The previous formula below applied the
                # RIGHT Hamilton product (q (x) [0, omega]), which is only correct for
                # BODY-frame omega (as used by the dof==4 spherical-joint branch just
                # below). Using it here silently mixed the two conventions: correct at
                # q = identity (both products coincide) but wrong thereafter, producing
                # an orientation error that grows with accumulated rotation and does NOT
                # vanish as dt -> 0 (BF-fix, verified against a closed-form isotropic
                # constant-omega rotation benchmark).
                qw, qx, qy, qz = q[s+3:s+7]
                ox, oy, oz = qdot_new[s+3:s+6]
                dqw = 0.5 * (-qx*ox - qy*oy - qz*oz)
                dqx = 0.5 * ( qw*ox + oy*qz - oz*qy)
                dqy = 0.5 * ( qw*oy + oz*qx - ox*qz)
                dqz = 0.5 * ( qw*oz + ox*qy - oy*qx)
