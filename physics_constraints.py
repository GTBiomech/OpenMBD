...
def _hysteresis_force(curve, penetration, pen_max, energy_retention=0.25,
    ...
    """
    Hysteresis Model:
      • Loading  (pen >= pen_max):  F = f_loading(pen)   [from ``curve``]
      • Unloading (pen < pen_max):
          - If ``unload_curve`` is provided: F = f_unloading(pen)
            Uses the explicit unloading characteristic from the JSON model,
            capturing the true energy-dissipation shape of each tissue region.
          - Fallback (no unload_curve): F = eta * f_loading(pen)
            where eta = energy_retention in [0, 1].
            Default eta = 0.25 -> 75% energy absorption per cycle,
            consistent with soft human tissue (Yamada 1973, PMHS data).

    Parameters
    ----------
    curve          : np.ndarray (N,2) – loading (penetration, force) table
    penetration    : float  – current depth
    pen_max        : float  – historical maximum depth for this contact pair
    energy_retention : float – eta fallback fraction when no unload_curve given
    unload_curve   : np.ndarray (N,2) or None – explicit unloading table
    """
    F_load = _elastic_force_from_curve(curve, penetration)
    if penetration >= pen_max:          # loading branch — always uses load curve
        return F_load
    else:                               # unloading branch
        if unload_curve is not None:
            return _elastic_force_from_curve(unload_curve, penetration)
        else:
...
class SimpleContact:
    ...
    def get_contact_force(self, penetration, v_rel_n):
        ...
        ua = getattr(self.ellipsoidA, 'unload_curve', None)
        ub = getattr(self.ellipsoidB, 'unload_curve', None)

        if ca is not None and cb is not None:
            curve = _get_combined_curve(ca, cb)
            # Combine unload curves the same way (series compliance on unloading)
